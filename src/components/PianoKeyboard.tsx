        ...
        {whiteKeys.map(key => (
          <div
            ...
            style={{ 
              ...
              boxShadow: key.isActive ? `inset 0 0 20px rgba(0,0,0,0.4)` : 'inset 0 -4px 6px rgba(0,0,0,0.1)'
            }}
            onMouseDown={() => onPlayNote?.(key.midi)}
            onMouseUp={() => onStopNote?.(key.midi)}
            onMouseLeave={() => onStopNote?.(key.midi)}
            onTouchStart={(e) => { e.preventDefault(); onPlayNote?.(key.midi); }}
            onTouchEnd={(e) => { e.preventDefault(); onStopNote?.(key.midi); }}
          >
        ...
        {blackKeys.map(key => (
          <div
            ...
            style={{ 
              ...
              boxShadow: key.isActive ? `0 0 15px ${key.color}` : 'inset -2px -4px 6px rgba(255,255,255,0.1)'
            }}
            onMouseDown={() => onPlayNote?.(key.midi)}
            onMouseUp={() => onStopNote?.(key.midi)}
            onMouseLeave={() => onStopNote?.(key.midi)}
