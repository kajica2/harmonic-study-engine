...
- `engine/` (repo root, PRD-001 Phase 0, ADR-003) = pure generative core: `core/` (rng, ids, versioned, spelling - ADR-010 hand-rolled, no tonal inside engine), `migrations/` (runner, v1->v2 session schema), `styles/` (StyleProfile data), `etude/` (Phase 3 slice 1: types/harmony/melody/assemble - seeded deterministic generators, draw-order contract harmony->melody->title, feasibilityOf conservative-sound invariant), `pedagogy/` (Phase 3 slice 1: 8 hand-curated concepts + TRUTHFUL annotation detectors per ADR-013). Direction rule: `src/** -> engine/**` allowed; `engine/** -> src/**` or any external package FORBIDDEN - enforced by `engine/purity.test.ts` (MIN_SCANNED_FILES=21, bans Math.random/Date.now/new Date/performance.now/require/console + non-relative imports in non-test sources).
