...
| Concept registry (>= 8 concepts) | SHIPPED - Phase 3 slice 1, 2026-09-23 (stale row fixed) | `engine/pedagogy/concepts.ts`: 10 hand-curated concepts (D17, REQ-PED-10/11; walking-bass + comping added in Phase 4 slice 3), every `exampleNumerals` parsing in the D21 grammar, references null by design (TD-037). The old note ("no structured Concept type") predates it. |
| Concept drawer | SHIPPED | Phase 3 slice 3: right-slide `ConceptDrawer` on `ModalShell` (REQ-PED-5), opened from etude annotation chips (and Compose analysis chips since Phase 4 slice 2); Phase 6, 2026-09-24: REQ-PED-13 footer ("Hear an example" = GENERATED exampleNumerals in C via the earHear transient path + "Send to Explore" seed carry) + header ConceptSearch global entry (REQ-PED-12, App-level host); PED-6 right-click/long-press/Shift+F10 SCOPED to AnalysisCard chord cells only (bare cells prefill global search), rest gated TD-PED-6-REST. |
| Ear training sub-mode | SHIPPED - Phase 6, 2026-09-24 | `EarTrainingPanel` SECTION on the Etude surface below the composer (not a tab; attempts never touch dirty): 7 seeded types (interval / chord-quality / chord-inversion / progression / scale / melodic-dictation / harmonic-dictation) x difficulty 1-5 x seed; same-pool distractors minus the answer enharmonically (Bb never distracts A#); pitch-class grading (Cb counts for B); partial credit melodic-dictation only (hit fraction + contour bonus, correct >= 0.85); miss offers the linked concept (REQ-PED-25). Q10 call: accuracy-% + streaks ("12/20 correct (60%) - streak 3"), NO XP/levels. Hear via the existing compose preview singleton. Docs: `docs/EAR-TRAINING.md`. |
| Spaced repetition | SHIPPED - Phase 6, 2026-09-24 | Canonical SM-2 (`engine/pedagogy/srs.ts`: ease 2.5 start / 1.3 floor, 1 then 6 then interval*ease, grade 5 correct / 2 incorrect) over localStorage `pedagogy.srs` (cap 10, idea-library pattern, no store v5); due-bias picker (overdue weighted, else longest-unseen). |
| Practice log | SHIPPED - Phase 6, 2026-09-24 | One entry per answer to `pedagogy.log` (REQ-PED-40/41/42, cap 500: mode/outcome/durationSec/conceptId) + pure summary derivation (minutes-per-day last 30d UTC, whole-log concept counts, last-20 rolling accuracy with partial = 0.5). Carve-out: NO on-screen summary panel yet (math ships, rendering is a fast-follow) - the panel shows the session honesty line only. |
